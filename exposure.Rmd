...
pl <- ggplot(control[taxa %in% high_taxa], aes(x=factor(Exposure), y=relative, fill=taxa)) + 
  ...
  facet_wrap(~ Response) + guides(fill = "none")
ggsave("figures/control_exposure.svg", height=4, width=1.5)
pl
```
...
pl <- ggplot(treated[taxa %in% high_taxa], aes(x=factor(Exposure), y=relative, fill=taxa)) + 
  ...
  labs(x="antibiotic exposure [days]", y="relative abundance", fill="phylum")
ggsave("figures/treatment_exposure.svg", height=6, width=8)
pl
```
...
```{r}
after_treat <- subset_samples(total, Day - Exposure > 0 & Day - Exposure < 10)
...
```{r}
ord <- rarefy_even_depth(after_treat, 15000) %>% ordinate(., method="PCoA", distance = "bray")
sda <- sample_data(after_treat)
sample_data(after_treat)$since_treatment <- factor(sda$Day - sda$Exposure)

pl <- plot_ordination(after_treat, ord, color = "Response", shape = "since_treatment") +
...
pl <- ggplot(status_counts, aes(x=response, y=n, group=exposure, color=exposure)) +
  ...
  labs(x="", y="num. ASVs", color="exposure [days]")
...
```{r}
status_counts[response %in% c("control", "non-responder"), 
              .(pval = wilcox.test(n ~ response, .SD)$p.value), 
              by=status]
```
...
```{r}
status_counts[response %in% c("non-responder", "responder"), 
              .(pval = wilcox.test(n ~ response, .SD)$p.value), 
              by=status]
```
...
ggplot(shannon[Day >= -1 & Day < 10], aes(x=Day, y=shannon, color=Response)) + 
