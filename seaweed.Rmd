...
pl <- plot_ordination(after_treat, ord, color = "Response", label="DaysElapsed") +
...
```{r}
fit <- aov(1/Ct ~ Response, data = sdata[DaysElapsed == 31])
summary(fit) %>% print
...
```{r}
fit <- aov(relative ~ Response, data = rel[DaysElapsed == 31])
summary(fit) %>% print
...
pl <- ggplot(weights, aes(x=day, y=weight, col=Response)) + 
  geom_vline(xintercept = c(20, 25, 31), lty="dashed") +
  geom_point(alpha=0.5) + geom_line(aes(group=Subject), alpha=0.5) + 
  facet_grid(seaweed ~ Response) + theme_bw() +
