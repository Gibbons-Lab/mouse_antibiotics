...
ref <- rarefy_even_depth(ref, 10000)
```
...
if (!file.exists("data/permanova_power.csv")) {
  pa <- power_analysis(ref, fraction_differential = 0.1,
                       threads = 16, n_power = 10, n_groups = 10, min_mu = 1e-3)
  power <- pa$power
...
if (!file.exists("data/mw_power.csv")) {
  collapsed <- collapse(ref, "Genus", 0)
  pa <- power_analysis(collapsed, 
                       fraction_differential = 0.5, method = "mw", threads=16, 
                       n_power=10, n_groups=10, min_mu = 1e-3)
  mw_power <- pa$power
...
```{r, fig.width = 4, fig.height = 3}
ggplot(power[effect > 0], aes(x = n, y = fdr, group=effect, color=factor(round(fold_change, 1)))) +
...
if (!file.exists("data/cc_power.csv")) {
  collapsed <- collapse(ref, "Genus", 0)
  pa <- power_analysis(
    collapsed, method = "corncob", threads=16, min_mu=1e-3, n_power = 10, n_groups = 10)
  cc_power <- pa$power
