...
seqtab <- readRDS(url(
  "https://data.mendeley.com/datasets/86xzh5c85p/1/files/da2d4b0c-4c8c-4a73-bfa1-4eb43a24c7d5/seqtab_final.rds?dl=1"))
taxa <- readRDS(url(
  "https://data.mendeley.com/datasets/86xzh5c85p/1/files/b98de0af-f296-4c17-84b4-46397fe9e36d/tax_final.rds?dl=1"))
meta <- read.csv(url(
  "https://data.mendeley.com/datasets/86xzh5c85p/1/files/4b44d05d-a341-4c6d-8420-feeef4fbe4db/meta.csv?dl=1"))
...
ggsave("figures/permanova_power.svg", width = 6, height = 4)
```
...
ggsave("figures/permanova_fdr.svg", width = 5, height = 4)
```
...
ggplot(power[effect > 0], aes(x = n, y = power, col = log10(mu/phi), group = taxa)) +
  scale_x_log10() +
  geom_hline(yintercept = 0.8, lty = "dashed") + 
  geom_point(alpha = 0.5) + geom_line(alpha = 0.5) +
  geom_linerange(aes(ymin = power - power_sd, ymax = power + power_sd),
                 alpha = 0.5) +
  facet_wrap(~ fold_change, labeller = label) + guides(color = FALSE)
ggsave("figures/mw_power.png", width = 7, height = 5, dpi = 300)
```
...
ggsave("figures/mw_fdr.svg", width = 6, height = 4)
```