...
counts <- log2(mbtools::normalize(t(count_mat[, meta$day >= 29])) + 0.5)
permanova <- adonis2(counts ~ type, data = meta[meta$day >= 29], 
                     method = "euclidean")
permanova
```
...
pheatmap(log10(norm_counts + 1), annotation_col = anns, 
         annotation_row = superpathways, color = viridis(128), 
         show_rownames = FALSE)
```
