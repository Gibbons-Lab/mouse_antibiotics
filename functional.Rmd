...
red[, "timepoint_cat" := "before"]
red[day >= 27, "timepoint_cat" := "during"]
...
counts <- log2(mbtools::normalize(t(count_mat)) + 0.5)
permanova <- adonis2(counts ~ day*type, data = meta, 
                     method = "euclidean")
permanova
```

We can also check for only respondersr vs controls before.

```{r}
permanova <- adonis2(counts[meta$day < 29, ] ~ type, data = meta[day < 29], 
                     method = "euclidean")
permanova
```
...
pl <- ggplot(tests, aes(x = log2FoldChange, y = -log10(pvalue), 
                        color = factor(day))) +
  ...
  theme_minimal() + theme(legend.position = c(0.15, 0.9), 
...
sig <- tests[padj < 0.05]
cn <- sig[, .(n=.N, coef=median(log2FoldChange)), by = c("subpathway", "coefficient", "day")][order(day, -n)]
ggplot(cn, aes(x=n, y=reorder(subpathway, n), fill=coefficient)) + geom_bar(stat="identity", position="stack")
```
...
names(cols$type) <- levels(anns$type)
